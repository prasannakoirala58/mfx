  ...
  const handleSubmit = async (e) => {
    ...
    for (let key in formData) {
      if (key === 'poster' || key === 'banner') {
        ...
        uploadData.set(key, blob, file.name);
      } else {
        uploadData.set(key, formData[key]);
      }
    }
    ...
    try {
      console.log('back ma request chai gayo hai');
...
const styles = {
  ...
  input:
    'block w-full text-sm text-gray-400 border border-gray-500 rounded-lg cursor-pointer bg-gray-800',
  ...
  btn_disabled:
    'w-full px-6 py-2.5 bg-blue-400 cursor-not-allowed text-white font-medium text-md rounded',
