...
const Shows = () => {
  ...
  const fetchShows = () => {
    ...
    axios
      .get(
        `/api/admin/shows/history?sortBy=${sortOption}&order=${sortOrder}&page=${page}&perPage=${rowsPerPage}`
      )
      .then((res) => {
        dispatch({
          type: 'FETCH_SUCCESS',
          payload: { ...res.data.data, loading: false, error: '' },
        });
...
const styles = {
  ...
  view_details_btn:
    'p-1 mx-1 bg-orange-600 cursor-pointer text-white text-center font-medium rounded-md',
  update_disabled_btn:
    'p-1 mx-1 cursor-not-allowed bg-purple-400 text-white text-center font-medium rounded-md',
  update_btn:
    'p-1 mx-1 bg-purple-700 cursor-pointer text-white text-center font-medium rounded-md',
};
