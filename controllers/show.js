...
exports.addNewShow = asyncHandler(async (req, res, next) => {
  ...
  if (show)
    return next(
      new CustomError('There is already a show for selected time & cinema hall 💥', 400)
    );
...
exports.updateShowDetails = asyncHandler(async (req, res, next) => {
  ...
  if (isAnyBooking)
    return next(new CustomError("Can't update this show, because there is booking", 400));
